	...
	if(!$resulttrees || !$resultgarden)
	...
	else
		{
			...
			if(mysqli_num_rows($resultgarden) > 0)
			{
				...
				{
					echo "<tr><td>",$row["productname"],"</td><td>",$row["productdesc"],"</td><td><form action='item.php' method='post'><input type='hidden' value='<?php echo $row['productname']; ?>', id='buyinggarden' '><button>Buy</button></form></td></tr>";
				}
				echo "</table>";
			}
		}
